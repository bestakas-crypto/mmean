...
### 8-1. 역할 분담 원칙 (2026-03-26 확정)

| 역할 | 담당 | 이유 |
|---|---|---|
| 진입 결정 | **엔진 전담** | 속도, BiasRegimeEngine 즉각 판단 |
| 손절 실행 | **엔진 전담** | 속도, sl_ticks 즉각 실행 |
| 익절 실행 | **엔진 전담** | 속도, tp_ticks / trailing |
| 청산 판단 지원 | **엔진 + LLM** | LLM은 "나와라" 신호만, "버텨라"는 신호 없음 |
| 파라미터 폭 결정 | **LLM** | sl_ticks, trailing_ticks, enter_score 범위 제안 |
| 큰 흐름 판단 | **LLM 10분** | MarketAnalyst (장중 view 이력) |

> **비대칭 권한 원칙**: LLM은 포지션을 늘리는 방향보다 줄이고 나오는 방향에 더 큰 권한을 가진다.
> `WA_MAX_UP = +0.10` (진입 확대 제한) vs `WA_MAX_DOWN = -0.20` (청산 방향 허용)
> 이 비대칭은 향후 청산 트리거 확장 시에도 동일하게 적용한다.

### 8-2. LLM이 할 수 있는 것 (v1 현재)
...
### 8-3. LLM이 할 수 없는 것 (v1 현재)

| 금지 항목 | 이유 |
|---|---|
| 진입 방향 결정 | BiasRegimeEngine 전담 |
| 진입 여부 차단 | CB 레이어 및 entry_gate 전담 |
| 청산 강제 (FORCE_EXIT_LLM) | 데이터 검증 없음 — v1 비활성 |
| 손익 기반 판단 | FORBIDDEN_REASON_CODES로 즉시 기각 |
| 보유 중 개입 | 진입 전 1회만 호출, 보유 중 호출 금지 |
| 수치 계산 (틱가격, PnL) | 엔진 전담, LLM 부동소수점 신뢰 불가 |

### 8-4. LLM 청산 권한 확장 로드맵

현재 v1에서 LLM의 청산 권한은 `weight_adjust` 음수 출력으로 간접 제한하는 수준이다.
직접 청산 트리거(FORCE_EXIT_LLM)는 아래 조건이 모두 충족된 후에만 활성화한다.

| 단계 | 조건 | 예상 시점 |
|---|---|---|
| 1 | market_view_log 1~2주 축적 | 2026-04월 초 |
| 2 | view 정확도 ≥ 65%, 허위 경보 ≤ 30% 검증 (A-010) | 2026-04월 초 |
| 3 | view → 청산 플래그 배선 shadow 운영 (A-011) | 2026-04월 중 |
| 4 | FORCE_EXIT_LLM 트리거 (T6) 구현 및 POLICY.md 갱신 (A-012) | 2026-04월 후 |

> **구현 전 절대 금지**: 검증 없이 LLM exit 배선을 활성화하면 허위 경보가 정상 수익 포지션을 강제 청산할 수 있다.
...
| 2026-03-26 | §8 LLM 역할 범위 전면 재작성. 역할 분담 표 / 비대칭 권한 원칙 / LLM 청산 로드맵 추가. |
